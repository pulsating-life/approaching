...
class FntMenuTablePage2 extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
            menuSet: {
	...
	initAppMenu = (fntMod) => {
		...
		if(this.refs.menuTree && fntMod.uuid){
			this.refs.menuTree.initTree(fntMod);
		}
	}

	onSelectMenu = (selNode, leafSet) => {
...
export default FntMenuTablePage2;
