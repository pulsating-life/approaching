...
class CreateCompUserPage extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			compUserSet: {
	...
	// 第一次加载
	componentDidMount() {
		this.unsubscribe = CompUserStore.listen(this.onCompUserComplete);
		this.unsubscribe2 = AuthUserStore.listen(this.onAuthUserComplete);
	...
	clear = (corpUuid, deptUuid) => {
        console.log(corpUuid, deptUuid);
