...
@ModalForm('userGroup')
class UpdateUserGroupPage extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			userGroupSet: {
			...
			>
		   		<Form layout={layout}>
                    ...
                    <FormItem {...formItemLayout} label="组说明" colon={true} className={layoutItem} help={hints.groupDescHint} validateStatus={hints.groupDescStatus}>
						<TextArea name="groupDesc" id="groupDesc" value={this.state.userGroup.groupDesc} onChange={this.handleOnChange}/>
					</FormItem>
				</Form>
			</Modal>
		);
