...
module.exports = {
    ...
    formatDate: function (value, dateFormat) {
        ...
        var mask = dateFormat ? dateFormat : 'YYYY-MM-DD';
    ...
    formatMonth: function (value, monthFormat) {
        ...
        var mask = monthFormat ? monthFormat : 'YYYY-MM';
        mask = mask.replace(/YYYY/g, yyyy);
        mask = mask.replace(/YY/g, yyyy.substring(2));
        mask = mask.replace(/MM/g, mm);
        mask = mask.replace(/MON/g, mon);
    ...
    formatTime: function (value, dateFormat) {
        ...
        var mask = dateFormat ? dateFormat : 'hh:mm:ss';
    ...
    formatShowText: function (type, value) {
        if ('date' === type) {
            return this.formatDate(value, this.dateFormat);
        }
        else if ('month' === type) {
            return this.formatMonth(value, this.monthFormat);
        }
        else if ('time' === type) {
            return this.formatTime(value, this.timeFormat);
        }
        else if ('amount' === type) {
            return this.formatAmount(value);
        }
        else if ('trim' === type) {
            return this.showTrimText(value);
        }

        return value;
    },
    formatSplitText: function (str) {
        if (str) {
            str = str.replace(/，/g, ',');
            str = str.replace(/；/g, ',');
            str = str.replace(/;/g, ',');
    ...
    initTreeNodes: function (recordSet, funcCrtNode) {
        var info = this.__prepareTreeNodes(recordSet, funcCrtNode);

        // 叶子节点
        var nodes = info.nodes;
        nodes.map((data, i) => {
            if (data.isLeaf !== true && data.isLeaf !== false) {
        ...
        return info.rootNodes;
    },

    // 两张表合并生成树,pDataSet=父节点,dataSet=子节点
    initTreeNodes2: function (pDataSet, funcCrtPnode, dataSet, funcCrtNode, delayLoad) {
        if (pDataSet.length === 0) {
            return [];
        }

        var info = this.__prepareTreeNodes(pDataSet, funcCrtPnode);
        var pNodeMap = info.nodeMap;
    ...
    __prepareTreeNodes: function (recordSet, funcCrtNode) {
        var nodes = [];
        var nodeMap = {};
        ...
        return {
            nodeMap: nodeMap, nodes: nodes, rootNodes: result
        };
    },
    ...
    // 生成目录
    prepareTreeNodes: function (pNode, disabledMap) {
        var disableCheckbox = pNode.disableCheckbox;
        var disabled = pNode.disabled;
        if (disabledMap) {
            var df = disabledMap[pNode.key];
            if (df) {
                disableCheckbox = true;
                disabled = true;
            }
        ...
        var title = pNode.icon ? <span><Icon type={pNode.icon} />{pNode.title}</span> : pNode.title;
        if (!pNode.isLeaf && pNode.children != null) {
            return (
                <TreeNode title={title} key={pNode.key} disabled={disabled} disableCheckbox={disableCheckbox} isLeaf={pNode.isLeaf}>
                    {
                        pNode.children.map((node1, i1) => {
                            return this.prepareTreeNodes(node1, disabledMap);
                        })
                    }
                </TreeNode>
            );
        ...
        for (var name in obj) {
            try {
                ...
                if (v !== null && typeof (v) === 'string') {
                    ...
                    if (v.indexOf(filterValue) >= 0 || filterReg.test(py)) {
                        return true;
                    }
                }
            } catch (E) { }
    ...
    sortTable2: function (recordSet, fieldName, dsc) {
        // 排序函数
        var f = null;
        if (dsc) {
            f = function (r1, r2) {
                var s1 = parseInt(r1[fieldName]);
                var s2 = parseInt(r2[fieldName]);
                return s2-s1;
            };
        }
        else {
    ...
    getLastDay: function (year, month) {
        ...
        nd = (new Date(nd.getTime() - 1000 * 60 * 60 * 24));
        return '' + (nd.getFullYear() * 10000 + (nd.getMonth() + 1) * 100 + nd.getDate());
    },
    registerEchartsThme: function (echarts) {
        theme.registerThme(echarts);
    },
};
...
var LzFormVerify = new FormVerify();
...
var _isValidMethod = [
    {
        type: 'int',
        isValid: function (str) { if (LzFormVerify.regInteger.test(str) == false) return '请输入整数!'; }
    },
    {
        type: 'double',
        isValid: function (str) { if (LzFormVerify.regDouble.test(str) == false) return '请输入Double数据，比如：55.069!'; }
    },
    {
        type: 'money',
        isValid: function (str) { if (LzFormVerify.regMoney.test(str) == false) return '请输入金额，比如：2355.00!'; }
    },
    {
        type: 'email',
        isValid: function (str) { if (LzFormVerify.regEmail.test(str) == false) return '请输入电子邮件地址!'; }
    },
    {
        type: 'idcard',
        isValid: isIdCard
    ...
    {
        type: 'phone',
        isValid: function (str) { if (LzFormVerify.regPhone.test(str) == false) return '请输入电话号码!'; }
    },
    {
        type: 'phones',
        isValid: function (str) { if (LzFormVerify.regPhones.test(str) == false) return '请输入电话号码，[,]分割!'; }
    },
    {
        type: 'mobile',
        isValid: function (str) { if (LzFormVerify.regMobile.test(str) == false) return '请输入手机号码!'; }
    },
    {
        type: 'zip',
        isValid: function (str) { if (LzFormVerify.regZip.test(str) == false) return '请输入六位数字的邮政编码!'; }
    },
    {
        type: 'url',
        isValid: function (str) { if (LzFormVerify.regUrl.test(str.toLowerCase()) == false) return '请输入HTTP地址!'; }
    },
    {
        type: 'classname',
        isValid: function (str) { if (LzFormVerify.className.test(str) == false) return '请输入有效的类名称!'; }
    },
    {
        type: 'qq',
        isValid: function (str) { if (LzFormVerify.regQQ.test(str) == false) return '请输入QQ号!'; }
    },
    {
        type: 'number',
        isValid: function (str) { if (LzFormVerify.regNumber.test(str) == false) return '请输入数字!'; }
    },
    {
        type: '汉字',
        isValid: function (str) { if (LzFormVerify.regChinese.test(str) == false) return '请输入汉字!'; }
    },
    {
        type: '字母',
        isValid: function (str) { if (LzFormVerify.regEnglish2.test(str) == false) return '请输入字母或空格!'; }
    }
];
...
function isIdCard(str) {
    var reg = (str.length == 18) ? LzFormVerify.regIdCard18 : LzFormVerify.regIdCard15;
