...
@ModalForm('confTask')
class CreateConfTaskPage extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
            loading: false,
	...
	clear = () => {
        this.state.hints = {};
        this.state.confTask = {};
        FormDef.initConfTaskForm(this.state.confTask);
        this.state.confTask.uuid = '';
	...
	onClickSave = () => {
        if (Common.formValidator(this, this.state.confTask)) {
            ...
            ConfTaskActions.createConfTask(this.state.confTask);
        }
	}
	...
	render(){
        ...
        return (
            <Modal visible={this.state.modal} width='640px' title="增加定时器配置信息" maskClosable={false} onOk={this.onClickSave} onCancel={this.toggle}
                footer={[
                    <div key="footerDiv" style={{ display: 'block', textAlign: 'right' }}>
                        <ServiceMsg ref='mxgBox' svcList={['conf_task/create']} />
...
export default CreateConfTaskPage;
